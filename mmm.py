...
# Admin user IDs
admin_id = {"6135948216"}  # Replace with actual admin ID
...
def generate_key(length=10):
    ...
    return f"ULTRA-BHAI-{key.upper()}"  # Ensure key is in uppercase
...
@bot.message_handler(commands=['ultra'])
def handle_attack(message):
    user_id = str(message.chat.id)
    cooldown_time = 0  # Cooldown in seconds (e.g., 0 seconds)
